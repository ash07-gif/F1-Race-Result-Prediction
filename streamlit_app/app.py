...
file_path = os.path.join(BASE_DIR, "backend", "f1_features.csv")

try:
    df = pd.read_csv(file_path)
except FileNotFoundError:
    st.error("❌ f1_features.csv not found in backend folder")
    st.stop()
...
# ✅ FIXED RANKING
test_df['predicted_rank'] = test_df['predicted_pos'].rank(method='first')
...
with tab2:
    ...
    st.bar_chart(test_df.set_index('driver')['positions_gained'])
    ...
    test_df['error'] = test_df['finish_pos'] - test_df['predicted_rank']
    st.bar_chart(test_df.set_index('driver')['error'])
...
with tab3:
    ...
    st.line_chart(driver_data.set_index('race')['finish_pos'])

    # 🔥 OVERTAKE SKILL
    st.markdown("## 🧠 Overtake Skill")
    skill = df.groupby("driver")["driver_overtake_skill"].mean()
    st.bar_chart(skill)